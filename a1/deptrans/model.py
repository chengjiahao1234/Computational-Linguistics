...
class ParserModel(nn.Module):
    ...
    def create_net_layers(self) -> None:
        ...
        as follows from the concatenated embedded input x:
            ...
            pred = h_drop W_o + b_o
        This method creates the two layers that create, initialize, and store
        the weights and biases W_h, b_h, W_o, and b_o for later use in
        computing the model prediction. Note that the layers will only compute
        result of the multiplication and addition (i.e., no activation function
        is applied, so the hidden layer will not apply the ReLU function).
    ...
    def forward(self,
        ...
        as follows from the concatenated embedded input x:
            ...
            h_drop = dropout(h, dropout_rate)
            pred = h_drop W_o + b_o
        This method computes pred from the inputs with the help of the other
        methods in this class. Note that, compared to the assignment handout,
        we've added dropout to the hidden layer and we will not be applying the
        softmax activation at all in this model code. See the get_loss method
        for why.
